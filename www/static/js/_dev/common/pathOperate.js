define(function(require, exports) {
	...
	var newFile = function(path,callback){
		...
		$.ajax({
			dataType:'json',
			url: '/explorer/mkfile?path='+urlEncode2(path),
			beforeSend:function(){
				core.tips.loading();
			},
	...
	var newFolder = function(path,callback){
		...
		$.ajax({
			dataType:'json',
			url: '/explorer/mkdir?path='+urlEncode2(path),
			beforeSend:function(){
				core.tips.loading();
			},
			error:core.ajaxError,
	...
	var past = function(path,callback){
		if (!path) return;
		var url='/explorer/pathPast?path='+urlEncode2(path);
		$.ajax({
			url:url,
			dataType:'json',
			beforeSend: function(){
	...
	var _chmod = function(dialog_id,param){
		$('.'+dialog_id).find('.edit_chmod').click(function(){
			...
			var $button = $(this);
			$.ajax({
				url:'/explorer/pathChmod?mod='+$input.val(),
				type:'POST',
				data:_json(param),	
				beforeSend: function(){
	...
	var unZip = function(path,callback){
		if (!path) return;
		var url='/explorer/unzip?path='+urlEncode2(path);
		$.ajax({
			url:url,
			beforeSend: function(){
