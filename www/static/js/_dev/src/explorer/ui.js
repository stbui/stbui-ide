define(function (require, exports) {
    ...
    var _f5 = function (is_data_server, is_animate, callback) {
        ...
        if (!is_data_server) {//采用当前数据刷新,用于显示模式更换
            ...
            $.ajax({
                url: '/explorer/pathlist/?path=' + urlEncode(G.this_path),
                dataType: 'json',
                beforeSend: function () {
    ...
    return {
        ...
        header: {
            ...
            addressSet: function () {
                ...
                // fix window路径斜杠
                var _path = $("input.path").val().replace(/\\/g, '/');
                $("input.path").val(_path);

                $("#yarnball").html(__set_address(_path));
                ui.header.set_width();
            },
            //自适应宽度
            set_width: function () {
